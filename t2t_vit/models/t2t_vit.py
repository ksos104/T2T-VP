...
class Inv_T2T_module(nn.Module):
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, embed_dim=768, token_dim=64):
        super().__init__()
        self.project = nn.Linear(embed_dim, token_dim * 3 * 3)
        self.soft_gather0 = nn.Fold(output_size=(8, 8), kernel_size=(3, 3), stride=(2, 2), padding=(1, 1))
        self.soft_gather1 = nn.Fold(output_size=(16, 16), kernel_size=(3, 3), stride=(2, 2), padding=(1, 1))
        self.soft_gather2 = nn.Fold(output_size=(64, 64), kernel_size=(7, 7), stride=(4, 4), padding=(2, 2))

        self.attention_inv1 = Token_transformer(dim=token_dim, in_dim=token_dim * 3 * 3, num_heads=1, mlp_ratio=1.0)
        self.attention_inv2 = Token_transformer(dim=token_dim, in_dim=in_chans * 7 * 7, num_heads=1, mlp_ratio=1.0)


    def forward(self, x):           ## x.shape = [1 10 16 384]
        B, T = x.shape[:2]
        x = einops.rearrange(x, 'b t n d -> (b t) n d')
        x = self.project(x)         ## x.shape = [10 16 576]
        x = einops.rearrange(x, 'bt n d -> bt d n')
        x = self.soft_gather0(x)    ## x.shape = [10 64 8 8]
        x = einops.rearrange(x, 'bt n h w -> bt (h w) n')
        x = self.attention_inv1(x)  ## x.shape = [10 64 576]
        x = einops.rearrange(x, 'bt hw n -> bt n hw')
        x = self.soft_gather1(x)    ## x.shape = [10 64 16 16]
        x = einops.rearrange(x, 'bt n h w -> bt (h w) n')
        x = self.attention_inv2(x)  ## x.shape = [10 256 49]
        x = einops.rearrange(x, 'bt hw n -> bt n hw')
        x = self.soft_gather2(x)    ## x.shape = [10 1 64 64]
        x = einops.rearrange(x, '(b t) n h w -> b t n h w', b=B, t=T)

        return x                ## x.shape = [1 10 1 64 64]
...
class T2T_ViT(nn.Module):
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        ## Decoder (Inverse T2T)
        self.decoder = Inv_T2T_module(img_size=img_size, tokens_type=tokens_type, in_chans=in_chans, embed_dim=embed_dim, token_dim=token_dim)
    ...
    def forward_features(self, x):
        ...
        '''
        '''
            transformer input dimention: (b n) t+1 d  --> 성능 낮음
        '''
        '''
    ...
    def forward(self, x):       ## input x (moving mnist) shape = [1 10 1 64 64]
        x = self.forward_features(x)
        # x = self.head(x)
        x = self.decoder(x)     ## input x (moving mnist) shape = [1 10 16 384]
        return x
...
def t2t_vit_t_14(pretrained=False, **kwargs):  # adopt transformers for tokens to token
    if pretrained:
        kwargs.setdefault('qk_scale', 384 ** -0.5)
    model = T2T_ViT(tokens_type='transformer', embed_dim=384, depth=14, num_heads=8, mlp_ratio=3., **kwargs)
