...
class recon_token_inference(nn.Module):
...
class pred_token_inference(nn.Module):
    def __init__(self, dim, num_heads=1, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
...
class token_Transformer(nn.Module):
    ...
    def forward(self, rgb_fea):
        ...
        recon_token = self.recon_token.expand(B, -1, -1)
        fea_1_16 = torch.cat((recon_token, fea_1_16), dim=1)

        pred_token = self.pred_token.expand(B, -1, -1)
        fea_1_16 = torch.cat((fea_1_16, pred_token), dim=1)
        # fea_1_16 [B, 1 + 14*14 + 1, 384]

        fea_1_16 = self.encoderlayer(fea_1_16)
        # fea_1_16 [B, 1 + 14*14 + 1, 384]
        recon_token = fea_1_16[:, 0, :].unsqueeze(1)
        pred_token = fea_1_16[:, -1, :].unsqueeze(1)

        recon_fea_1_16 = self.recon_token_pre(fea_1_16)
        pred_fea_1_16 = self.pred_token_pre(fea_1_16)
        return recon_fea_1_16, fea_1_16, recon_token, pred_fea_1_16, pred_token
